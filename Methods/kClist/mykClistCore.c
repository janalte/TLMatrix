...
bheapLLU *constructLLU(unsigned n_max){
 ...
 void swapLLU(bheapLLU *heap,unsigned i, unsigned j) {
 ...
 void bubble_upLLU(bheapLLU *heap,unsigned i) {
	unsigned j=(i-1)/2;
	while (i>0) {
		if (heap->kv[j].value>heap->kv[i].value) {
			swapLLU(heap,i,j);
 ...
 void bubble_downLLU(bheapLLU *heap) {
	unsigned i=0,j1=1,j2=2,j;
	while (j1<heap->n) {
		j=( (j2<heap->n) && (heap->kv[j2].value<heap->kv[j1].value) ) ? j2 : j1 ;
		if (heap->kv[j].value < heap->kv[i].value) {
 ...
 void insertLLU(bheapLLU *heap,keyvalueLLU kv){
	heap->pt[kv.key]=(heap->n)++;
	heap->kv[heap->n-1]=kv;
	bubble_upLLU(heap,heap->n-1);
}

 void updateLLU(bheapLLU *heap,unsigned key,unsigned long long delta){
	unsigned i=heap->pt[key];
	if (i!=-1){
 ...
 keyvalueLLU popminLLU(bheapLLU *heap){
...
int main(){
	...
	nck=onepass(g,kmax);
	char* output1="./phoneckdeg.txt";
	printf("Writing %u-cliques degree of each node in file: %s\n",kmax, output1);
	tot=printckdeg(nck,g,output1);
	tot/=kmax;
	t2=time(NULL);
	printf("- Time = %ldh%ldm%lds\n",(t2-t1)/3600,((t2-t1)%3600)/60,((t2-t1)%60));
	t1=t2;
