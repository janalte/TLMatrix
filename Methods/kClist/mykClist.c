...
void print_triangle(unsigned u, unsigned v, unsigned w, FILE* fp) {
    // printf("Triangle: %u %u %u\n", u, v, w);
	fprintf(fp, "%u,%u,%u\n", u, v, w);
}

void kclique(unsigned l, specialsparse *g, unsigned long long *n, FILE*fp) {
	...
	for(i=0; i<g->ns[l]; i++){
		...
		if (l == 3) { // special handling for triangles
            for (j = 0; j < g->ns[2]; j++) {
                v = g->sub[2][j];
                end = g->cd[v] + g->d[2][v];
                for (k = g->cd[v]; k < end; k++) {
...
int main(){
	...
	char* outname = "./a2q_kclist.csv";

	FILE *file = fopen(outname, "w"); // 打开文件用于写入
    if (file == NULL) {
        printf("无法打开文件\n");
        return 1;
    }
	...
	char* filename="./dataset/a2q_data.txt";
